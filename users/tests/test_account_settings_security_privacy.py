...
class AccountSettingsSecurityPrivacyTest(TestCase):
    ...
    def test_account_settings_renders_security_privacy_controls_and_real_gmail_status(self):
        ...
        self.assertNotContains(response, "Scan scope")
        self.assertNotContains(response, "Retention period")
        self.assertNotContains(response, "Automatic scans")
        self.assertNotContains(response, "connected@gmail.com")
        self.assertNotContains(response, "Token healthy")
        self.assertNotContains(response, "https://www.googleapis.com/auth/gmail.readonly")
        self.assertNotContains(response, reverse("accounts:resync_gmail", kwargs={"connection_id": connection.id}))
        self.assertNotContains(response, reverse("accounts:revoke_gmail", kwargs={"connection_id": connection.id}))
    ...
    def test_gmail_resync_revoke_and_privacy_controls_post_card_level_feedback(self):
        ...
        self.assertContains(resync_response, "Gmail integrations")
        self.assertContains(revoke_response, "Gmail access revoked.")
        self.assertContains(revoke_response, "Gmail integrations")
